...
${article.title != void 0 ? `<div class="${"bg-careersBandTopBg bg-cover bg-center bg-no-repeat p-8 pb-12 flex flex-wrap pt-12 justify-center"}"><div class="${"mt-8 mb-12 w-full md:w-5/6 justify-center items-center flex flex-wrap"}"><div class="${"w-full md:w-1/3 xl:w-1/4"}"><img class="${"mx-auto"}"${add_attribute("src", img.logo, 0)}></div>
      <div class="${"w-full md:w-2/3 xl:w-3/4 pl-0 md:px-16"}"><h1>Contact Us</h1>
        <img class="${"pb-4"}"${add_attribute("src", img.divider, 0)}>
        <div class="${"text-textGray style-dato-header"}"><!-- HTML_TAG_START -->${render(article.description)}<!-- HTML_TAG_END --></div></div></div></div>
        ...
        <div class="${"w-full md:w-1/2 mt-8 md:mt-0"}"><h1>Newsletter Signup</h1>
          <form id="${"signup"}" method="${"post"}" action="${"https://www.aweber.com/scripts/addlead.pl"}"><input type="${"hidden"}" name="${"listname"}" value="${"awlist6157223"}">
            ...
            <label class="${"mt-4 block text-sm font-body text-textGray"}">Name
              <input type="${"text"}" class="${"mt-1 w-full text-sm p-1 text-charcoal rounded-md"}" name="${"name"}" value="${""}"></label>
            <label class="${"mt-4 block text-sm font-body text-textGray"}">Email
              <input type="${"text"}" class="${"mt-1 w-full text-sm p-1 text-charcoal rounded-md"}" name="${"email"}" value="${""}"></label>
