<script>
  ...
  let socials = [
    [ "", "/img/socials/social-discord.svg" ],
    [ "", "/img/socials/social-twitter.svg" ],
...
<div class="bg-transparent {fullyLoaded == false ? "hidden" : "flex flex-wrap"}">
  ...
  <div class="w-full relative">
    { #if navSelected != "/about" && navSelected != "/games" }
      <div class="z-20 w-full bottom-[110px] absolute h-[80px] footer-gradient"></div>
    { /if }
    <div class="z-30 mission-mask w-full h-[120px] flex flex-wrap items-center relative -mt-8">
      <div class="absolute bg-tan bg-center md:bg-cover bg-no-repeat w-full h-full"></div>
      <div class="pt-2 px-8 absolute w-full h-full flex flex-wrap items-center">
        <div class="w-1/6 xl:w-[50%] leading-8 text-left flex flex-wrap items-center">      
          <img class="mb-0 mt-1 h-[30px]" src={logoImg}/>
          { #if winWidth > 1360 }
            <span class="w-full lg:w-auto font-body text-textGray text-tiny pl-0 lg:pl-2">&#169; Riyo Inc 2022, All Rights Reserved | <a class="text-tiny underline" href="/privacy">Privacy Policy</a> | Site Design by Fully Illustrated</span>   
          { /if }
        </div>
        { #if winWidth >= 768 }
        <div class="w-5/6 xl:w-[50%] text-right">
          <div class="hidden md:inline-block">
            ...
            { #each socials as social }
              <a href={social[0]}><img class="ml-2 inline-block w-[30px] h-auto" src={social[1]}/></a>
            {/each}
          </div>
        </div>
        { :else }
        ...
        { /if }
        { #if winWidth <= 1360 }
          <span class="w-full font-body text-textGray text-tiny">&#169; Riyo Inc 2022, All Rights Reserved | <a class="text-tiny underline" href="/privacy">Privacy Policy</a> | Site Design by Fully Illustrated</span>   
        { /if }
      </div>
    </div>
  </div>
